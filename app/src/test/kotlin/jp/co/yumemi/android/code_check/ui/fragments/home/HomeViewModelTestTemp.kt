...
class HomeViewModelTestTemp {
    ...
    @Test
    fun `getGitHubRepoList success`() = testDispatcher.runBlockingTest {
        // Arrange
        val inputText = "example"
        `when`(networkInfo.isConnected).thenReturn(true)
        `when`(connectivityManager.activeNetworkInfo).thenReturn(networkInfo)
        ...
        // Ensure that the progress dialog is shown
        assert(viewModel.isDialogVisible.value == true)

        // Advance the coroutine dispatcher to execute coroutine logic
        testDispatcher.scheduler.advanceUntilIdle()

        // Ensure that the progress dialog is hidden
        assert(viewModel.isDialogVisible.value == false)

        // Ensure that the GitHubRepoList is updated
        assert(viewModel.gitHubRepoList.value == MockObjects.expectedData)

        // Ensure that the error message is not set
